...
const fetchTenders = async () => {
  try {
    setLoading(true);
    let url = `/Tender?page=${page + 1}&limit=${rowsPerPage}`;
 ...
 // Handle form input changes
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEditing) {
        await api.put(`/Tender/${currentTender._id}`, payload);
        setSnackbar({ open: true, message: 'Tender updated successfully', severity: 'success' });
      } else {
        await api.post('/Tender', payload);
  ...
  const toggleActiveStatus = async (id, currentStatus) => {
    try {
      await api.put(`/Tender/${id}`, { isActive: !currentStatus });
      setSnackbar({ 
        open: true, 
        message: `Tender ${currentStatus ? 'deactivated' : 'activated'} successfully`, 
        severity: 'success' 
      });
      fetchTenders();
    } catch (error) {
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 1 }}>
            ...
            <Box>
              ...
              <FileUploadComponent 
                ...
                defaultFolder="/public/Tenders/Additional"
              />
            </Box>
